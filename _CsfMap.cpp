...
void cTMap::GetMapHeader(QString Name)
{
  MAP *m = Mopen(Name.toLatin1().constData(), M_READ);
  if (m == NULL)
    vError(QString("Map %1 cannot be opened.").arg(Name));
  //   {
...
void cTMap::CreateMap(QString Name)
{
  ...
  if (Data == NULL)
    vError(QString("Cannot create data structure for map: %1").arg(Name));
  //   {
...
bool cTMap::LoadFromFile()
{
  ...
  if (!fi.exists())
    vError(QString("Map %1 does not exist.").arg(PathName));
  //   {
